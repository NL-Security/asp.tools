    ...
    <div class="w3-bar">
        ...
        <div class="w3-dropdown-hover">
            ...
            <div class="w3-dropdown-content w3-bar-block w3-card-4 w3-theme">
                <%=Writer.Write("html.navitem(""edit?" & primaryKey("COLUMN_NAME") & "="" & " & table_name & "(""" & primaryKey("COLUMN_NAME") & """)).label(strings(""modifier"")).css(""w3-theme"")") %><%=vbcrlf %>
                <%=vbtab %><%=vbtab %><%=vbtab %><%=vbtab %><%=Writer.Write("html.deleteitem(""delete?" & primaryKey("COLUMN_NAME") & "="" & " & table_name & "(""" & primaryKey("COLUMN_NAME") & """)).css(""w3-theme"")") %>
            </div>
        </div>
    </div>
</div>
<div class="w3-row w3-padding">
    ...
    <div class="w3-col m9">
        ...
        <%=writer.Start %>set <%=writer.Pluralize(foreignKeys("FK_TABLE_NAME")) %> = db.entity("<%=foreignKeys("FK_TABLE_NAME") %>").where("<%=foreignKeys("FK_COLUMN_NAME") %> = " & <%=table_name %>("<%=primaryKey("COLUMN_NAME") %>"))<%=schemaservice.sqljoin(foreignKeys("FK_TABLE_NAME")) %>.list<%=writer.Terminate %>
